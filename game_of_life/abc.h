...
{
public:
	abc(int birth = 0): x(rand() % 40), y(rand() % 60), birthday(birth) {}
	abc(int x_cur, int y_cur, int birth = 0) : x(x_cur), y(y_cur), birthday(birth) {}
	virtual ~abc() {}
...
private:
	...
	int y;
	int birthday;
	const int life_day = 7;
	const int reproduction_day = 3;
};
